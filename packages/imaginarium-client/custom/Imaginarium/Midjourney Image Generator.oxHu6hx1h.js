...
module.exports = (node, graph) => {
  ...
  const imageElementCreator = ImageElementCreator({ origin: configuration.serverOrigin });
  ...
  async function generateImageAndRenderCanvas() {
    ...
    const imageElement = await imageElementCreator.midjourneyImageGenerationToImageElement({
      prompt: prompt,
      width: 512,
      height: 512
    });
  ...
  const generateImageInput = node.in('Midjourney: Generate Image', generateImageAndRenderCanvas, { connectable: false });
};